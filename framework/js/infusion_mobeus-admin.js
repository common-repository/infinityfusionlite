...
j(document).ready(function($){
  ...
  if(hash !== ''){
    ...
    $('.infusion_mobeus-sidebar ul li a[href="'+hash+'"]').addClass('active');
  } else {
    $('.infusion_mobeus-main-content .page:first').show();
    $('.infusion_mobeus-sidebar ul li a:first').addClass('active');
  }

  $('.infusion_mobeus-sidebar ul li a').bind('click', function(){
    $('.infusion_mobeus-content .page').hide();
    var lock = $(this).attr('href');
    $('#page-' + lock.replace(/#/, '')).fadeIn(45).show();
    $('.infusion_mobeus-sidebar ul li a').removeClass('active');
    $(this).addClass('active');
  });

  $('#infusion_mobeus-admin-wrap #infusion_mobeus-form').submit(function(){
    var form = $(this);
    form.trigger('infusion_mobeus-before-save');
    var button = $('#infusion_mobeus-admin-wrap #save-button');
    ...
    $.post(form.attr("action"), form.serialize(), function(data){
      ...
      form.trigger('infusion_mobeus-saved');
    }, 'json');
    return false;
  });

  $('#infusion_mobeus-admin-wrap #reset-button').live('click', function(){
    if(confirm('Click to reset. All settings will be lost.')){
      $(this).val('Resetting...');
      $.post(ajaxurl, {action: 'infusion_mobeus_framework_reset', nonce:$("#infusion_mobeus_noncename").val() }, function(data){
        if(data.error){
  ...
  // Layout Switcher
  $("#infusion_mobeus-admin-wrap .main-layout br").remove();
  $('#infusion_mobeus-admin-wrap .main-layout input[type="radio"]').each(function(){
    var label = $(this).parent();
    label.addClass($(this).val());
    if($(this).is(':checked')) label.addClass('checked');
  });
  $('#infusion_mobeus-admin-wrap .main-layout label').live('click', function(){
    $("#infusion_mobeus-admin-wrap .main-layout label").removeClass('checked');
    $('#infusion_mobeus-admin-wrap .main-layout input[type="radio"]').attr('checked', false);
    var id = $(this).attr('for');
    $(this).addClass('checked');
    $('#infusion_mobeus-admin-wrap .main-layout #'+id).attr('checked', true);
  });

  var hasAdded = false;
  $("#infusion_mobeus-form input, #infusion_mobeus-form textarea, #infusion_mobeus-form select").live("keyup change input paste", function(){
    if(hasAdded === false){
      $("<span class='changed'>There are Unsaved Changes!</span>").insertBefore("#infusion_mobeus-admin-wrap");
    }
    hasAdded = true;
  });
  ...
  $("#download_export_data").click(function(e){
    e.preventDefault();
    var code = localStorage.getItem("infusion_mobeus_theme_settings_data");
  ...
  $("#download_debug_info").click(function(e){
    e.preventDefault();
    var code = localStorage.getItem("infusion_mobeus_debug_info_data");
    if(typeof(code) === 'undefined'){
      $("output").text("Unable to geneated text file due to some error, please contact to the support team.");
      return false;
    }else{
  ...
  localStorage.setItem("infusion_mobeus_theme_settings_data", $("#theme_settings_data").text());
  localStorage.setItem("infusion_mobeus_debug_info_data", $("#debug_info_data").text());
...
var downloadFile = function(code, id){
  ...
  var time = new Date();
  var filename = "infusion_mobeus_"+time.getMonth()+"/"+time.getDate()+"/"+time.getFullYear()+"-"+time.getHours()+":"+time.getMinutes()+":"+time.getSeconds();
